...
const ServiceDetail: React.FC = () => {
    ...
    const loading = useSelector((state: RootState) => state.serviceDetail.loading);
  ...
  return (
    <>
      <DetailItem 
        ...
        nameThir={serviceDetail?.devices?.map((d) => d.name).join(", ") || "N/A"}
        priceMin={serviceDetail?.durations?.length
          ? serviceDetail.durations.reduce((min, d) =>
              d.price < min.price ? d : min
            ).price
          : 0 }
        priceMax={serviceDetail?.durations?.length
